...
import { dummyFlashSaleData } from "../data/dummy-flash-sale-data";
...
const ProductsFlashSaleBlock: React.FC<ProductsProps> = ({
  ...
  const displayData = error || !data ? dummyFlashSaleData : data;
  return (
    <ProductFlashSaleWrapper
      sectionHeading={sectionHeading}
      className={className}
    ...
    >
      ...
      >
        {limit ? (
          <>
            {displayData?.productFlashSellGridTwo
              ?.slice(0, limit)
              ?.map((product: any) => (
                <ProductCard
          ...
          <>
            {displayData?.productFlashSellGridTwo?.map((product: any) => (
              <ProductCard
                key={`product--key${product.id}`}
                product={product}
                imgWidth={itemVariant === "list" ? 180 : 324}
